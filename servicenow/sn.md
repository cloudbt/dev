...
```
**① 性能改善対応（1/23リリース）**
- 試したこと：時間のかかるSW情報データマッピング処理を15分間隔の対象から外し、HW情報のみを処理
- 結果：それでも完了までに約1時間を要する状況

**② 短縮策の検討**
- 試したこと：Last Seen（最終検知日時）を基に直近30分以内の端末に絞り込む案を検討
- 結果：Last Seenが09:00、10:00、11:00…のように1時間単位で一律に更新されており、端末ごとのランダムな時刻になっていないことが判明

**③ Tanium社への確認（青木様・有田様経由）**
- 結果：Tanium Asset（ServiceNowと直接連携するデータ）は1時間ごとにしか更新されない仕様であることが判明
