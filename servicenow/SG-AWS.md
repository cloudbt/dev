...
Diagnostic Summary:
- Config / SSM / Image API = **Organization全体459アカウント中、8アカウントのみ成功**
　→ SG-AWS連携対象アカウント観点では、**対象14アカウント中、6アカウントのみ成功**
- Warning メッセージ: アカウント数459、リージョン数1のため AWS Config aggregator の利用が推奨（Note #17）
　→ ServiceNowと連携する対象AWSアカウント数は少ないため、**Config aggregator の導入は不要**と判断しています。
